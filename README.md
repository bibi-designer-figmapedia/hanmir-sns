...
Claude Code 서브 에이전트 2개 + MCP 2개(Apify·Figma).

```
BRAND.md ──▶ ig-competitor-scout ──▶ 보고서 .pptx
                    │ 게이트 ①
                    ▼
...
```
BRAND.md                 브랜드 설명서 7칸 (빈 템플릿) — 자기 브랜드로 쓴다 (★ 제품·이미지에 나올 것·경쟁사 필수 · 1번 브랜드 컬러를 적으면 피그마 버튼·강조색이 그 색으로)
...
```bash
npm install
claude            # /mcp 에서 apify · figma connected 확인
```
